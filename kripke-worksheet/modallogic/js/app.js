/**
 * Based on rkirsling/modallogic (MIT License).
 * Original: https://github.com/rkirsling/modallogic
 * Local modifications for teaching worksheet:
 * - Propositional letters changed to uppercase
 * - Small focus/keybinding tweaks for iframe embedding
 */
...
var svg = d3.select('#app-body .graph')
  .append('svg')
  .attr('tabindex', 0)  // make SVG focusable so it can receive key events
...
function mousedown() {
  ...
  // ensure the SVG has keyboard focus whenever you click in the graph
  var svgNode = svg.node();
  if (svgNode && svgNode.focus) {
    svgNode.focus();
  }
...
function keydown() {
  ...
  switch(d3.event.keyCode) {
    ...
    case 76: // L
      ...
      break;
    case 82: // R
      console.log('R pressed; selected_node?', !!selected_node, 'selected_link?', !!selected_link);
      if(selected_node) {
...
function setAppMode(newMode) {
  ...
  svg.classed('edit', true)
    ...
    .on('mouseup', mouseup)
    .on('keydown', keydown) // listen for keys on the focused SVG
    .on('keyup', keyup);
